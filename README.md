  ...
  - **GUI (main.py, main.ui, ui.py)**  
    ...
    - **main.py**: Main entry point. Loads the UI, sets up event handlers (button clicks), and manages the application lifecycle. Connect button to python function
    - **main.ui**: GUI designed using PySide6 Designer, layout. You define where the buttons are (t, f, ^, v, =, Clear), what their names are (e.g., button_clear), and how the grid looks.
    - **ui.py**: Generated Python code from `main.ui`.

    updating ui.py, run in root
    ```sh
    pyside6-uic src/components/main.ui -o src/components/ui.py
    ```
